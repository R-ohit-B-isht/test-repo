...
const SORT_STRATEGIES = {
  ...
  blades: (a, b) => b.scores.blades - a.scores.blades || overallScore(b) - overallScore(a) || a.price - b.price,
...
const state = { sort: "score", type: "all", maxPrice: 10000 };
...
function filteredPacks() {
  return PACKS
    ...
    .filter((s) => {
      ...
      if (state.type === "runtime90") return runtimeOfSpec(s) >= 90;
      if (state.type === "waterproof") return /waterproof|ipx/i.test(s.fullSpec.waterproof);
      if (state.type === "usbc") return /USB-C/.test(s.fullSpec.chargeType);
      if (state.type === "zerogap") return /yes/i.test(s.fullSpec.zeroGap);
...
function runtimeOfSpec(s) {
  const m = (s.fullSpec.runtime || "").match(/(\d+)/);
...
const FULL_SPEC_SECTIONS = [
  {
    ...
    fields: {
      ...
      cordUse: "Corded / cordless",
    },
  },
  {
    title: "Blades & Precision",
    fields: {
      blades: "Blade material",
      selfSharpening: "Self-sharpening",
      zeroGap: "Zero-gap blades",
  ...
  },
  {
    title: "Usability",
    fields: {
      waterproof: "Waterproof / washable",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "runtime" ? ' data-sort="battery"' : ""}${c.key === "blades" ? ' data-sort="blades"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} trimmers`;
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(PACKS.map((s) => ({
      ...
      runtime: s.fullSpec.runtime,
      blades: s.fullSpec.blades,
      store: s.buyStore,
    })));
  }
}
