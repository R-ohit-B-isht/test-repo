...
const SORT_STRATEGIES = {
  score: (a, b) => overallScore(b) - overallScore(a) || b.scores.insulation - a.scores.insulation || a.price - b.price,
  insulation: (a, b) => b.scores.insulation - a.scores.insulation || overallScore(b) - overallScore(a) || a.price - b.price,
  lid: (a, b) => b.scores.lid - a.scores.lid || overallScore(b) - overallScore(a) || a.price - b.price,
  capacity: (a, b) => b.scores.capacity - a.scores.capacity || overallScore(b) - overallScore(a) || a.price - b.price,
...
const state = { sort: "score", type: "all", maxPrice: 1000 };

function filteredTumblers() {
  return TUMBLERS
    .filter((s) => s.price <= state.maxPrice)
    .filter((s) => {
      if (state.type === "all") return true;
      if (state.type === "steel") return /steel/i.test(s.fullSpec.material);
      if (state.type === "insulated") return /vacuum|double.?wall|insulat/i.test(s.fullSpec.insulation);
      if (state.type === "straw") return /straw/i.test(s.fullSpec.lidType);
      if (state.type === "big") return parseInt(s.fullSpec.capacity) >= 900;
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "insulation" ? ' data-sort="insulation"' : ""}${c.key === "capacity" ? ' data-sort="capacity"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(TUMBLERS.map((s) => ({
      model: `${s.brand} ${s.model}`,
      price: s.price,
      score: overallScore(s),
      insulation: s.scores.insulation,
      capacity: s.fullSpec.capacity,
      material: s.fullSpec.material,
      store: s.buyStore,
    })));
  }
}
