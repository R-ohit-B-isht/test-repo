...
const SORT_STRATEGIES = {
  ...
  safety: (a, b) => b.scores.safety - a.scores.safety || overallScore(b) - overallScore(a) || a.price - b.price,
...
const state = { sort: "score", type: "all", maxPrice: 14000 };

function filteredCooktops() {
  return COOKTOPS
    .filter((s) => s.price <= state.maxPrice)
    .filter((s) => {
      if (state.type === "all") return true;
      if (state.type === "w2000") return /(2[0-9]{3})\s*W/i.test(s.fullSpec.wattage) && parseInt(s.fullSpec.wattage) >= 2000;
      if (state.type === "touch") return /feather|touch/i.test(s.fullSpec.controlType) && !/push/i.test(s.fullSpec.controlType);
      if (state.type === "preset") return /\d/.test(s.fullSpec.presetMenus) && parseInt(s.fullSpec.presetMenus) >= 5;
      if (state.type === "autooff") return /yes/i.test(s.fullSpec.autoShutOff);
      return true;
    })
...
function cardHtml(s, i) {
  ...
  return `<article class="card ${topPick ? "top-pick" : ""}">
    ...
    <p class="specs"><strong>${s.wattage}</strong> \u00B7 ${s.controls}<br>
      ${s.safetyLine}<br>Presets: ${s.presets}</p>
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "wattage" ? ' data-sort="power"' : ""}${c.key === "autoShutOff" ? ' data-sort="safety"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(COOKTOPS.map((s) => ({
      model: `${s.brand} ${s.model}`,
      price: s.price,
      score: overallScore(s),
      power: s.scores.power,
      wattage: s.fullSpec.wattage,
      controls: s.fullSpec.controlType,
