...
const SORT_STRATEGIES = {
  ...
  ignition: (a, b) => b.scores.ignition - a.scores.ignition || overallScore(b) - overallScore(a) || a.price - b.price,
...
function filteredPacks() {
  return PACKS
    ...
    .filter((s) => {
      ...
      if (state.type === "dualarc") return /dual|double/i.test(s.fullSpec.arcType);
      if (state.type === "windproof") return /windproof|flameless/i.test(s.fullSpec.windproof) && !/not stated/i.test(s.fullSpec.windproof);
      if (state.type === "usbc") return /USB-C/.test(s.fullSpec.chargeType);
      if (state.type === "neck") return /long|extended|neck|candle/i.test(s.fullSpec.formFactor);
      if (state.type === "display") return /display|indicator/i.test(s.fullSpec.display) && !/not stated/i.test(s.fullSpec.display);
...
const FULL_SPEC_SECTIONS = [
  {
    title: "Ignition",
    fields: {
      arcType: "Arc / ignition type",
      windproof: "Windproof / flameless",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "battery" ? ' data-sort="battery"' : ""}${c.key === "arcType" ? ' data-sort="ignition"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} lighters`;
