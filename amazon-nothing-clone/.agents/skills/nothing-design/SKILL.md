---
name: nothing-design
description: Use when the user explicitly says "Nothing style", "Nothing design", "/nothing-design", or directly asks to use or apply the Nothing design system. Works for any AI agent or coding assistant that can read skill instructions. NEVER trigger automatically for generic UI or design tasks.
---
...
**Before starting any design work, declare which Nothing font assets are required and how to load them** (see `references/tokens.md` Section 1). Never assume fonts are already available.
...
| Layer | What | How |
|-------|------|-----|
| **Primary** | The ONE thing the user sees first. A number, a headline, a state. | Ndot 57 or NType 82 at display size. `--text-display`. 48–96px breathing room. |
| **Secondary** | Supporting context. Labels, descriptions, related data. | NType 82 at body/subheading. `--text-primary`. Grouped tight (8–16px) to the primary. |
| **Tertiary** | Metadata, navigation, system info. Visible but never competing. | NType 82 at caption/label. `--text-secondary` or `--text-disabled`. ALL CAPS. Pushed to edges or bottom. |
...
- **2 font families** (NType 82 + Ndot 57. Ndot 57 only for hero moments.)
...
**Be consistent in:** Font families, label treatment (always NType 82 ALL CAPS), spacing rhythm, color roles, component shapes, alignment.

**Break the pattern in exactly ONE place per screen:** An oversized number, a circular widget among rectangles, a red accent among grays, an Ndot 57 headline, a vast gap where everything else is tight.
...
3. **Data as beauty.** `36GB/s` in NType 82 at 48px IS the visual. No illustrations needed.
...
|------|----------|--------|
| Hero number (large Ndot 57/NType 82) | Single key metric | Heavy — use once |
...
1. **Declare fonts** — tell the user which Nothing font assets to load (see `references/tokens.md`)
