...
All buttons: `NType 82`, 13px, ALL CAPS, letter-spacing 0.06em, padding 12px 24px. Min height 44px.
...
- Label above: `--label` style (NType 82, ALL CAPS, `--text-secondary`)
- Focus: border → `--text-primary`. Error: border → `--accent`, message below in `--accent`
- Data-entry fields: `NType 82` for input text
...
- Left: label (NType 82 caps, `--text-secondary`). Right: value (`--text-primary`)
- Never alternating row backgrounds. Use dividers.

**Stat rows:** Label left (NType 82, ALL CAPS, `--text-secondary`), value right (color = status color), unit adjacent in `--label` size. Trend arrow same color as value.
...
- Cell text: `NType 82` numeric, `NType 82` text. Cell padding: 12px 16px
...
- Labels: NType 82, ALL CAPS. Active: `--text-display` + dot/underline. Inactive: `--text-disabled`
...
- Border: `1px solid --border-visible`, no fill. Text: NType 82, `--caption`, ALL CAPS
...
- Text: NType 82, ALL CAPS, `--label` size. Height: 36–44px. Transition: 200ms ease-out
- Max 2–4 segments
...
- Label: NType 82, ALL CAPS. Arrows: thin chevrons, `--text-secondary`, 44px touch
...
**Charts:** Line 1.5–2px `--text-display`, average dashed 1px `--text-secondary`. Axis labels: NType 82, `--caption`. Grid: `--border`, horizontal only. No area fill, no legend boxes — label lines directly.

---

## 13. WIDGETS (DASHBOARD CARDS)

- `--surface` bg, 16px radius. Hero metric: large Ndot 57/NType 82, left-aligned
- Unit: `--label` size, adjacent. Category: ALL CAPS NType 82 top-left
...
- **Toasts:** None. Use inline status text: `[SAVED]`, `[ERROR: ...]`. NType 82, `--caption`, near trigger.
