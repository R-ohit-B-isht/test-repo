...
|------|------|----------|--------|
| **Display** | `"Ndot 57"` | `"NType 82", monospace` | 400 |
| **Body / UI** | `"NType 82"` | system-ui, sans-serif | Regular 400 |
| **Data / Labels** | `"NType 82"` | `"SF Mono", monospace` | Regular 400 |

**Why these fonts:** `Ndot 57` and `NType 82` are the actual Nothing font family names. Use local or project-provided licensed font files such as `NDot-57.otf` and `NType82-Regular.otf`; do not substitute Google Fonts when the real brand fonts are available.

### Loading Requirements

- Verify the project has licensed copies of `NDot-57.otf` and `NType82-Regular.otf` before implementing final UI.
- Do not bundle, publish, or copy the font files into public outputs unless redistribution rights are explicitly confirmed.
- If the real fonts are unavailable, tell the user and use fallbacks only for temporary previews.
...
- **Ndot 57:** 36px+ only, tight tracking, never for body text
- **Labels:** Always NType 82, ALL CAPS, 0.06–0.1em spacing, 11–12px ("instrument panel" labels)
- **Data/Numbers:** Always NType 82. Units as `--label` size, slightly raised, adjacent
- **Hierarchy:** display (Ndot 57) > heading (NType 82) > label (NType 82 caps) > body (NType 82). Four levels max.
...
**When to use:** Hero typography (Ndot 57), decorative grid backgrounds, dot-grid data viz, loading indicators, empty state illustrations.
