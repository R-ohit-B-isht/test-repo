...
import type { EvidenceStatus, FamilyKey } from '../../lib/types';
import { scoreClass, usePrefersReducedMotion, verdict } from '../../lib/format';
import { EVIDENCE_META, FAMILY_TONE, TONE_TEXT } from '../../domain/scoreMeta';

/** Hub family chip (power / grooming / kitchen …): a coloured dot + the manifest's family label. */
export function FamilyBadge({ family, label, className }: { family: FamilyKey; label: string; className?: string }) {
  return (
    <span className={clsx('label inline-flex items-center gap-1.5', FAMILY_TONE[family].text, className)}>
...
/** The listing's segment (capacity class, wattage band …) as a neutral dot chip; "unstated" reads muted. */
export function SegmentBadge({ id, label, className }: { id: string; label: string; className?: string }) {
  return (
    <span className={clsx('label inline-flex items-center gap-1.5', id === 'unstated' ? 'text-muted' : 'text-primary', className)}>
      <span className={clsx('h-2 w-2 rounded-full', id === 'unstated' ? 'bg-line-strong' : 'bg-accent')} aria-hidden />
      {label}
    </span>
  );
}

/** What the score was read from: maker page (with the number of verified fields), spec table, claims only, or nothing. */
export function EvidenceBadge({ status, verified, className }: { status: EvidenceStatus; verified?: number; className?: string }) {
  const meta = EVIDENCE_META[status];
  const Icon = status === 'official' ? ShieldCheck : status === 'listing' ? Table2 : ShieldAlert;
  return (
    <span className={clsx('label inline-flex items-center gap-1', TONE_TEXT[meta.tone], className)} title={meta.label}>
      <Icon size={11} aria-hidden />
      {meta.short}
      {status === 'official' && verified !== undefined && verified > 0 && <span className="font-medium text-secondary">· {verified} field{verified === 1 ? '' : 's'}</span>}
