...
import { SORT_OPTIONS, type SortKey } from '../../domain/sort';
...
interface Props {
  idx: CategoryIndex; segmentKey: string; featured: string[]; selected: string[]; live: { base: Uint32Array; byGroup: Map<string, Uint32Array> };
...
export function Toolbar({ idx, segmentKey, featured, selected, live, sort, onSort, query, onQuery, onToggle, onOpenFilters, activeCount, resultCount }: Props) {
  ...
  const quick = featured.filter((tag) => idx.tagPos.has(tag) && !tag.startsWith(`${segmentKey}:`));
  return (
