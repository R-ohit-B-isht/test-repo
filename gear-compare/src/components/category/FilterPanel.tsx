...
interface Props {
  ...
  order: string[];
  /** The segment group is rendered as the segmented control above the list, not repeated here. */
  segmentKey: string;
...
/** The first few category facets (schema order) and the evidence tier start expanded; the rest collapse. */
const OPEN_FIRST = 3;
const ALWAYS_OPEN = new Set(['ev']);
const SHOW_LIMIT = 8;

/** Multi-select facet panel. OR within a group (switchable to "match all" for multi-valued fields such as ports / protections), AND across groups. */
export function FilterPanel({ idx, groups, order, segmentKey, live, state, onToggle, onToggleAll, onClearGroup, onPrice }: Props) {
  const priceCap = Math.ceil(idx.priceMax / 100) * 100;
  const visible = order.filter((g) => g !== segmentKey && groups[g] && idx.facets[g]?.length);
  return (
    <div className="space-y-2">
      ...
      {visible.map((g, i) => (
        <FacetGroup key={g} id={g} def={groups[g]} rows={idx.facets[g]} idx={idx} liveCounts={live.byGroup.get(g) ?? live.base} state={state}
          defaultOpen={i < OPEN_FIRST || ALWAYS_OPEN.has(g)} onToggle={onToggle} onToggleAll={onToggleAll} onClear={onClearGroup} />
      ))}
    </div>
  );
}
...
interface GroupProps {
  id: string; def: FacetGroupDef; rows: FacetRow[]; idx: CategoryIndex; liveCounts: Uint32Array; state: ViewState; defaultOpen: boolean;
  onToggle: (tag: string) => void; onToggleAll: (g: string) => void; onClear: (g: string) => void;
}

function FacetGroup({ id, def, rows, idx, liveCounts, state, defaultOpen, onToggle, onToggleAll, onClear }: GroupProps) {
