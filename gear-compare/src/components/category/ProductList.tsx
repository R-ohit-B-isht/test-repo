...
interface Props {
  ...
  /** Planner hook-up (organiser categories only): ids already in the plan and the add / remove action. */
  plan?: { planned: ReadonlySet<string>; onPlan: (id: string) => void };
}
...
export function ProductList({ idx, segment, positions, compare, compareMax, onOpen, onCompare, plan }: Props) {
        ...
        return (
          <li key={row.id} data-index={vr.index} ref={v.measureElement} className="absolute left-0 top-0 w-full" style={{ transform: `translateY(${vr.start - margin}px)` }}>
            <ProductCard row={row} rank={idx.rank[pos]} segment={segmentOf(row.t)}
              ...
              plan={plan ? { planned: plan.planned.has(row.id), onPlan: plan.onPlan } : undefined} />
          </li>
        );
      })}
    </ol>
