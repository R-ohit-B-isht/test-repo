...
import { FamilyBadge, Kicker, NumberTicker, SkeletonRows, StatusBlock } from '../components/ui/primitives';
import { EmptyResults } from '../components/category/EmptyResults';
import { Sheet } from '../components/ui/Sheet';
import { SegmentControl } from '../components/category/SegmentControl';
...
function CategoryView({ id }: { id: string }) {
  ...
  const segKey = meta?.segment.key ?? 'seg';
  const segSelected = state.tags.filter((t) => t.startsWith(`${segKey}:`));
  ...
  useDevPublish(isDev, {
    ...
    evidence: meta ? JSON.stringify(meta.evidence) : null,
  ...
  const segmentOf = segmentResolver(idx, meta.segment);
  ...
  const panel = (
    <FilterPanel idx={idx} groups={groups} order={meta.facets} segmentKey={segKey} live={live} state={state}
    ...
    <div className="pb-24">
      <header className="pt-6 sm:pt-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <AppLink to="/" className="inline-flex items-center gap-1 text-[13px] font-bold text-secondary no-underline hover:text-display"><ChevronLeft size={14} />All categories</AppLink>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3"><Kicker>{meta.kicker}</Kicker><FamilyBadge family={meta.family} label={m.families[meta.family]} /></div>
          ...
          <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
            {([['Listings', idx.items.length], ['Brands', idx.brands.length], ['Flipkart', meta.stores.flipkart], ['Amazon.in', meta.stores.amazon]] as const).map(([k, n]) => (
              <div key={k} className="card px-4 py-3 sm:min-w-[110px]"><dt className="label">{k}</dt><dd className="mt-0.5 text-[24px] font-extrabold text-display sm:text-[28px]"><NumberTicker value={n} /></dd></div>
            ))}
          </dl>
        </div>
      ...
      <div className="glass sticky top-16 z-30 -mx-4 mt-8 border-b border-line px-4 py-3 sm:-mx-6 sm:px-6">
        <SegmentControl segment={meta.segment} rows={idx.facets[segKey] ?? []} selected={segSelected} onToggle={toggleTag} onClear={() => clearGroup(segKey)} />
      </div>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <Toolbar idx={idx} segmentKey={segKey} featured={meta.featured} selected={state.tags} live={live} sort={state.sort} onSort={(k) => update({ sort: k })}
          ...
          <ActiveChips idx={idx} groups={groups} state={state} onRemove={toggleTag} onPrice={onPrice} onQuery={onQuery} onClearAll={clearAllWithUndo} />
          {positions.length === 0 ? (
      ...
      <CompareTray category={id} shards={m.shards} fields={meta.fields} rows={compareRows} ranks={compareRows.map((r) => rankOf(r.id))} onRemove={onCompare} onClear={() => setCompare([])}
