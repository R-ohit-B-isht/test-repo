...
// Every URL here was fetched and checked (HTTP 200, product/image confirmed) on the date in `checked`. Facts are read
// off the maker's own product page; `evidence` lists independent reviews that support the pick.
// Adding a category: run `node scripts/build-data.mjs` — it fails if any URL/image/fact is missing.
export default [
  {
