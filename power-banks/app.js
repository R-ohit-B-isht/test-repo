...
const SORT_STRATEGIES = {
  ...
  speed: (a, b) => b.scores.speed - a.scores.speed || overallScore(b) - overallScore(a) || a.price - b.price,
...
function filteredPacks() {
  return PACKS
    ...
    .filter((s) => {
      ...
      if (state.type === "big") return mahOfSpec(s) >= 20000;
      if (state.type === "huge") return mahOfSpec(s) >= 30000;
      if (state.type === "fast") return wattOfSpec(s) >= 33;
      if (state.type === "pd") return /PD/.test(s.fullSpec.fastCharge);
      if (state.type === "display") return /display/i.test(s.fullSpec.display) && !/not stated/i.test(s.fullSpec.display);
...
function mahOfSpec(s) {
  const m = (s.fullSpec.capacity || "").replace(/,/g, "").match(/(\d+)/);
  return m ? parseInt(m[1]) : 0;
}
function wattOfSpec(s) {
  const m = (s.fullSpec.output || "").match(/([\d.]+)/);
...
const FULL_SPEC_SECTIONS = [
  {
    ...
    fields: {
      capacity: "Capacity",
      output: "Max output",
      fastCharge: "Fast-charge standard",
      cellType: "Cell type",
    },
  },
  {
    title: "Ports & Convenience",
    fields: {
      typeC: "USB Type-C",
      ports: "Output ports",
      display: "Charge display",
      passthrough: "Pass-through charging",
    },
  },
  {
    title: "Safety & Body",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "capacity" ? ' data-sort="capacity"' : ""}${c.key === "output" ? ' data-sort="speed"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} power banks`;
