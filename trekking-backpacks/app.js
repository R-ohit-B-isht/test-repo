...
const SORT_STRATEGIES = {
  score: (a, b) => overallScore(b) - overallScore(a) || b.scores.comfort - a.scores.comfort || a.price - b.price,
  comfort: (a, b) => b.scores.comfort - a.scores.comfort || overallScore(b) - overallScore(a) || a.price - b.price,
  weather: (a, b) => b.scores.weather - a.scores.weather || overallScore(b) - overallScore(a) || a.price - b.price,
...
const FULL_SPEC_SECTIONS = [
  ...
  },
  {
    title: "Comfort & Carry",
    fields: {
      frame: "Frame / back support",
      padding: "Shoulder & back padding",
      hipBelt: "Hip belt",
      chestStrap: "Chest strap",
    },
  },
  {
    title: "Weather & Organisation",
    fields: {
      rainCover: "Rain cover",
      waterResist: "Water resistance",
      compartments: "Compartments",
      pockets: "External pockets",
    },
  },
  {
    title: "General",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "capacity" ? ' data-sort="capacity"' : ""}${c.key === "rainCover" ? ' data-sort="weather"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} backpacks`;
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(PACKS.map((s) => ({
      ...
      comfort: s.scores.comfort,
      capacity: s.fullSpec.capacity,
      rainCover: s.fullSpec.rainCover,
      store: s.buyStore,
    })));
  }
}
