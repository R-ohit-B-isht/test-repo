// Rendering + interaction for category ranking pages. Strategy pattern for sorting.
// Expects: CTPRODUCTS (per-category data file) and window.CT_LABEL (plural noun for counts).
...
function filteredItems() {
  return CTPRODUCTS
    ...
    .filter((s) => {
      ...
      if (state.type === "derm") return s.filters.derm;
      if (state.type === "natural") return s.filters.natural;
...
const FULL_SPEC_SECTIONS = [
  {
    title: "Format & Value",
    fields: { quantity: "Quantity", pricePer100: "Price per 100g/ml", format: "Format" },
  },
  {
    title: "Skin Safety",
    fields: { freeFrom: "Free-from claims", dermTested: "Derm tested", nonComedogenic: "Non-comedogenic", skinType: "Skin type" },
  },
  {
    title: "Actives & Claims",
    fields: { keyIngredients: "Key ingredients", benefit: "Benefit claims", naturalClaim: "Natural claim" },
  },
  {
    title: "General",
    fields: { audience: "Marketed for", rating: "User rating", madeIn: "Made in" },
  },
];
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} ${window.CT_LABEL || "products"}`;
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(CTPRODUCTS.slice(0, 200).map((s) => ({
      model: `${s.brand} ${s.model}`,
      price: s.price,
      score: overallScore(s),
      trust: s.scores.trust,
      quantity: s.fullSpec.quantity,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("heroCount").textContent = CTPRODUCTS.length.toLocaleString("en-IN");
  initDevMode();
  render();
});
