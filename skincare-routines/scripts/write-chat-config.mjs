 ...
 *
 *   GEMINI_API_KEY=… SITE_URL=https://… REMINDERS_API_BASE=https://… node scripts/write-chat-config.mjs [dist-dir]
 *
 * REMINDERS_API_BASE (optional) is the origin of the chat-api deployment that runs the push-reminder loop; left empty,
 * reminders call the same origin and the Reminders panel reports push as unavailable if nothing answers there.
 */
...
const remindersApiBase = (process.env.REMINDERS_API_BASE ?? '').replace(/\/$/, '');
...
console.log(`wrote ${path.join(dist, 'chat-config.json')} (mode=browser, model=${config.gemini.model}, key=${key.slice(0, 6)}…${key.slice(-4)}, siteUrl=${siteUrl || '(same origin)'}, reminders=${remindersApiBase || '(same origin)'})`);
