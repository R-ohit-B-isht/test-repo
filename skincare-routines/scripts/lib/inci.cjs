...
function isMultiProduct(text) {
  ...
  const single = splitRaw(text.replace(/\s*\(and\)[^,;]*/gi, '')).filter((t) => !CODE_LIKE.test(t)).map(canonical);
...
function classify(text, opts = {}) {
  ...
  const rec = tokens.filter((t, i) => known[i] || INCI_LIKE.test(t)).length;
  const strictN = tokens.filter((t, i) => known[i] || (INCI_LIKE.test(t) && !GENERIC_WORD.test(t))).length;
