...
function filteredPacks() {
  return WASHES
    ...
    .filter((s) => {
      ...
      if (state.type === "sulfatefree") return /sulfate|sls/i.test(s.fullSpec.freeFrom) && !/not stated/i.test(s.fullSpec.freeFrom);
      if (state.type === "parabenfree") return /paraben/i.test(s.fullSpec.freeFrom) && !/not stated/i.test(s.fullSpec.freeFrom);
      if (state.type === "moisturizing") return /moisturi/i.test(s.fullSpec.skinBenefit) && !/not stated/i.test(s.fullSpec.skinBenefit);
      if (state.type === "exfoliating") return /exfoliat/i.test(s.fullSpec.skinBenefit) && !/not stated/i.test(s.fullSpec.skinBenefit);
      if (state.type === "natural") return !/not stated/i.test(s.fullSpec.naturalClaim);
      if (state.type === "men") return /for men/i.test(s.fullSpec.audience);
      if (state.type === "women") return /for women/i.test(s.fullSpec.audience);
...
const FULL_SPEC_SECTIONS = [
  ...
  {
    title: "Ingredients & Feel",
    fields: {
      keyIngredients: "Key ingredients",
      skinBenefit: "Skin benefit",
      fragrance: "Fragrance",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "freeFrom" ? ' data-sort="skin"' : ""}${c.key === "pricePer100ml" ? ' data-sort="value"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} washes`;
...
function initDevMode() {
  ...
  if (params.get("dev") === "1") {
    ...
    console.table(WASHES.map((s) => ({
      model: `${s.brand} ${s.model}`,
      price: s.price,
      score: overallScore(s),
      trust: s.scores.trust,
      volume: s.fullSpec.volume,
      freeFrom: s.fullSpec.freeFrom,
      store: s.buyStore,
    })));
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("heroCount").textContent = WASHES.length.toLocaleString("en-IN");
  initDevMode();
  render();
});
