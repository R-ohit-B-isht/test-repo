...
/** Explore-style "show me…" prompt (Screenroom command-input + ⌘K): finds a category, a face/body slice of it, or searches inside the current page. */
export function CommandSearch() {
  ...
  const all = useMemo(() => (manifest.status === 'ready' ? buildCommands(manifest.data.categories, currentCategory, dev) : []), [manifest, currentCategory, dev]);
          ...
          <motion.div className="fixed inset-0 z-[60] flex items-start justify-center bg-[rgba(26,23,29,0.45)] px-4 pt-[10vh] backdrop-blur-[2px]" onClick={() => setOpen(false)}
            ...
            <motion.div role="dialog" aria-modal="true" aria-label="Show me" onClick={(e) => e.stopPropagation()}
              ...
              <ul ref={list} id="cmd-list" role="listbox" className="scrollbar-thin max-h-[52vh] overflow-y-auto p-1.5">
                {results.length === 0 && <li className="px-3 py-6 text-center text-[14px] text-secondary">Nothing matches “{q}”. Try a category name, “face” or “body”.</li>}
...
function groupBy(cmds: Command[]): [string, Command[]][] {
  const order = ['Categories', 'Face or body only', 'Protocols', 'Just browsing', 'On this page'];
