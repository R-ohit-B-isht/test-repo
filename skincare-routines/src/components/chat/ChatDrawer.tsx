...
import { closeChat, deleteConversation, newChat, openConversation, retryLast, send, showChat, showHistory, stopChat, useChat } from '../../chat/chatStore';
...
export function ChatDrawer() {
  ...
  const headerExtra = view === 'history'
    ? <button type="button" onClick={showChat} className="btn h-10 gap-1.5 px-3 text-[12px]" aria-label="Back to chat"><ArrowLeft size={14} /> Back</button>
    : (
      <>
        <button type="button" onClick={showHistory} className="btn h-10 w-10 px-0" aria-label={`Chat history (${conversations.length})`} title="Chat history"><History size={15} /></button>
        {messages.length > 0 && <button type="button" onClick={newChat} className="btn h-10 w-10 px-0" aria-label="New chat" title="New chat"><SquarePen size={15} /></button>}
      </>
    );
      ...
      footer={view === 'history' ? undefined :
        <div className="space-y-1.5">
          ...
          <p className="label flex items-center justify-between gap-3 px-1 text-[11px] font-semibold text-muted">
            ...
            <span className="shrink-0"><span className="hidden sm:inline">Gemini · </span>product facts from site data</span>
          </p>
        </div>
      }>
      <div ref={scroller} className="scrollbar-thin flex-1 overflow-y-auto px-5 py-5 sm:px-6">
        {view === 'history' ? (
          <HistoryList items={conversations} activeId={activeId} storageOk={storageOk} onOpen={openConversation} onDelete={deleteConversation} />
        ) : messages.length === 0 ? (
