  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <p className="mt-2 text-[13.5px] leading-relaxed text-secondary">
        Gemini reads the same data as these pages{manifest ? ` — ${manifest.total.toLocaleString('en-IN')} live Flipkart, Amazon.in & brand-store listings across ${manifest.categories.length} categories, dataset ${new Date(manifest.generatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}` : ''}.
