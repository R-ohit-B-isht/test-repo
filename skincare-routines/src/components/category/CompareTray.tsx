...
function DetailRows({ category, shards, rows }: { category: string; shards: number; rows: ProductRow[] }) {
  ...
  const shownInSummary = new Set(['quantity', 'volume', 'format', 'pricePer100', 'pricePer100ml', 'rating']);
  const keys = new Set<string>();
  for (const d of details) if (d.status === 'ready') for (const k of Object.keys(d.data.fullSpec)) if (!shownInSummary.has(k)) keys.add(k);
  return (
    <>
      {[...keys].map((k) => (
        <Row key={k} label={specLabel(k)}>
          {details.map((d, i) => (
