...
- Parts of the routine ("my hair routine", "add brushing teeth at night", "move this to the body tab", "what do I do for my face on Monday?"): each step's zone in the context (face, body, scalp, lengths, beard, oral, other) is where it goes, and My routine groups them as tabs — Face, Body, Hair (scalp + lengths + beard), Teeth (oral), Other; when a step's tab differs from its zone (read from its category or title, e.g. a 'Brush teeth' step saved under face shows on the Teeth tab) the context says so in brackets. Answer per part by reading the zones; to move a step between parts, call edit_routine_steps with op move (or update) and zone. The site ranks no oral-care or "other" products, so those steps carry no category or listing: propose them with category "" and no product_id, and moving a step there needs product_id "none" alongside when a listing is pinned. Never say the routine cannot hold teeth / nails / other steps — it can, as name + days + note.
...
export const routineStepLine = (s: RoutineStepContext) =>
  `- [step ${s.id}] ${s.slot.toUpperCase()} #${s.position} · ${s.days.join('/')} · ${s.zone}${s.part && s.part !== s.zone ? ` (${s.part} tab)` : ''}: ${s.title}${s.category ? ` (category ${s.category})` : ''}`
  + (s.product ? ` — ${s.product.brand} ${s.product.title} [[${s.product.id}]]${s.product.rank != null ? ` #${s.product.rank}` : ''}` : ' — no product yet')
  + (s.withMe === false ? ' · not with me' : '')
  + (s.rotation ? ` · rotates weekly: ${s.rotation.options.map((o, i) => `${i + 1} ${o.title}${o.product ? ` (${o.product.brand} ${o.product.title} [[${o.product.id}]])` : ' (no product)'}`).join(' · ')} (this week: option ${s.rotation.active})` : '')
...
export function systemInstruction(m: Manifest, siteUrl: string): string {
  ...
  const inci = m.inci ? `- Ingredient search: ${fmt(m.inci.verified)} verified INCI lists searchable by any ingredient name via get_top_products \`ingredients\` (${fmt(m.inci.claimed)} more listings only have a seller line — reported as unverified)\n` : '';
...
export function pageContextBlock(page: PageContext | null): string {
  ...
  if (page.routineSteps?.length) {
    lines.push(`The user's saved routine (${page.routineSteps.length} steps, listed in application order — #n is the step's position within its AM/PM slot; use the step id with edit_routine_steps):`);
  ...
  if (page.routineReminders) {
    const r = page.routineReminders;
    lines.push(`reminders: morning ${r.am.enabled ? 'on' : 'off'} ${r.am.time} · night ${r.pm.enabled ? 'on' : 'off'} ${r.pm.time} (change with set_reminders)`);
  }
