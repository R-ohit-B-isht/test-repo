...
def page_context_block(page: dict) -> str:
    ...
    if isinstance(steps, list) and steps:
        lines.append(f"The user's saved routine ({len(steps)} steps, listed in application order — #n is the step's position within its AM/PM slot; use the step id with edit_routine_steps):")
...
def routine_step_line(step: dict) -> str:
    ...
    position = f" #{step['position']}" if isinstance(step.get("position"), int) else ""
    part = f" ({step.get('part')} tab)" if step.get("part") and step.get("part") != step.get("zone") else ""
    line = f"- [step {step.get('id')}] {str(step.get('slot', '')).upper()}{position} · {days} · {step.get('zone')}{part}: {step.get('title')}"
