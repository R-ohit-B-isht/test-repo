...
const SORT_STRATEGIES = {
  ...
  care: (a, b) => b.scores.care - a.scores.care || overallScore(b) - overallScore(a) || a.price - b.price,
...
const state = { sort: "score", type: "all", maxPrice: 15000 };
...
function filteredPacks() {
  return PACKS
    ...
    .filter((s) => {
      ...
      if (state.type === "highwatt") return parseInt(s.fullSpec.wattage, 10) >= 1800;
      if (state.type === "ionic") return /ionic|ion /i.test(s.fullSpec.tech) && !/not stated/i.test(s.fullSpec.tech);
      if (state.type === "coolshot") return /cool/i.test(s.fullSpec.coolShot) && !/not stated/i.test(s.fullSpec.coolShot);
      if (state.type === "foldable") return /fold/i.test(s.fullSpec.foldable) && !/not stated/i.test(s.fullSpec.foldable);
      if (state.type === "diffuser") return /diffuser/i.test(s.fullSpec.attachments) && !/not stated/i.test(s.fullSpec.attachments);
...
const FULL_SPEC_SECTIONS = [
  ...
  {
    title: "Hair Care Tech",
    fields: {
      tech: "Ionic / ceramic tech",
      heatProtect: "Heat protection",
    },
  },
  {
    title: "Build & Handling",
    fields: {
      foldable: "Foldable",
...
function tableHtml(items) {
  ...
  const head = `<tr>
    ...
    ${specCols.map((c) => `<th${c.key === "wattage" ? ' data-sort="power"' : ""}${c.key === "tech" ? ' data-sort="care"' : ""}>${c.label}</th>`).join("")}
    <th>Buy</th></tr>`;
  const rows = items
...
function render() {
  ...
  document.getElementById("count").textContent = `${items.length} dryers`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("heroCount").textContent = PACKS.length.toLocaleString("en-IN");
  initDevMode();
  render();
});
