...
import { lovedBy, totalHearts } from '../votes.js';
...
const actLine = (x, state, plan) => {
  ...
  const hearts = lovedBy(state, x.id);
  return [x.id, x.name, x.stop, isFun(x) ? 'fun' : 'see', `${x.slot}/${x.h}h`, inr == null ? 'free' : `₹${inr}${x.est ? '~' : ''}`, state.picks[x.id] ? 'ON' : 'off', where, flag(x.must, '★must'), flag(hearts.length, `♥${hearts.length}:${hearts.join('+')}`), flag(x.tag, `#${x.tag}`), flag(x.est, 'gemini-added'), x.note || ''].filter(Boolean).join(' | ');
};
...
export function buildContext(state) {
  ...
  return [
    ...
    totalHearts(state) ? `VOTES: ♥n:names = who hearted it (${totalHearts(state)} hearts). More hearts = more people want it.` : 'VOTES: nobody has hearted anything yet.',
    `DAYS:`,
...
export const SYSTEM = [
  ...
  'When asked to resolve votes: switch ON what most people hearted, make room by switching off paid picks nobody hearted, keep ★must unless it has zero hearts and something hearted needs its slot, and name who loses out in "say".',
